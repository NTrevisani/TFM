...
VQE:
===================================
python mkPlot.py 10 mean     22 VQE
python mkPlot.py 10 cvar_0.5 22 VQE
python mkPlot.py 10 cvar_0.2 22 VQE
                                VQE
python mkPlot.py 11 mean     27 VQE
python mkPlot.py 11 cvar_0.5 27 VQE
python mkPlot.py 11 cvar_0.2 27 VQE
                                VQE
python mkPlot.py 12 mean     33 VQE
python mkPlot.py 12 cvar_0.5 33 VQE
python mkPlot.py 12 cvar_0.2 33 VQE
                                VQE
python mkPlot.py 13 mean     39 VQE
python mkPlot.py 13 cvar_0.5 39 VQE
python mkPlot.py 13 cvar_0.2 39 VQE

QAOA:
====================================
python mkPlot.py 10 mean     22 QAOA
"""
...
from Functions import cost_function_C, QAOA_circuit, VQE_circuit, cost_function_cobyla, time_vs_shots
...
W = random_graph_producer(n, E, seed, verbosity=True)

# Solve the Max-Cut problem using brute-force approach
# and save the solution
brute_solution, brute_cost, eigenvalues = brute_force_solver(W, verbosity=True)
...
# (Brute cost is positive)
save_name = folder_name + "rel_diff_vs_inv_shots"
y_unc_rel = 1 / np.sqrt(N_repetitions)

scatter_plot(x        = 1 / np.sqrt(df_plot.shots),
             #y        = 1 + df_plot.cost/brute_cost,
             y        = (mean_eig + df_plot.cost) / (abs(mean_eig - brute_cost)),
             #do_fit   = True,
             #fit_func = "1-exp",
             title    = r"Relative difference with mean cost function value vs $\frac{1}{\sqrt{Shots}}$",
             xlabel   = r"$1 / \sqrt{Shots}$",
             ylabel   = "Relative difference with mean cost function value",
             y_err    = (1 + df_plot.cost/brute_cost) * y_unc_rel,
             ylim     = (-(brute_cost - mean_eig) / (abs(mean_eig - brute_cost)),
                         0.5*(brute_cost - mean_eig) / (abs(mean_eig - brute_cost))),
             save_as  = save_name)
